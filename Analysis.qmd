...
par(mfrow = c(1,1), mar = c(5,10,5,3))
...
# But add back number_of_lineages
metricsToPlot[1] = TRUE
...
x = barplot(t((predictabilityR2CV)), beside = T, las = 1, horiz = F, ylim = c(-.1,1.1), ylab = expression("Predicted-observed " ~ R^2), xaxt = "n", cex.lab = 1.8, cex.axis = 1.6, col = modelColors$color)
...
set.seed(1)
envCP = crossPredictability(processIndex = 1, modelsToExclude = c(5,7,8), iterations = 100)

comCP = crossPredictability(processIndex = 5, modelsToExclude = c(4,7), iterations = 100)
