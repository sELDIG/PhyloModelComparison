...
NOTE FLORIAN: seems a bit redundant to write and read again and there are minimal numeric differences and factor difference when using all.equal(allSimuData,allSimuData2) but I will leave it for the moment 
...
models = as.list(predictabilityR2CV)
...
rownames(predictability2) = predictors
colnames(predictability2) = models

RFimportancePlot(predictability2)
```
...
Run random forests within each simulation model ito predict model parameter values from simulation-derived tree shape metrics. Then plug in empirical tree shape metrics to infer a process parameter value assuming that the tree arose from the processes represented by a particular simulation model.
