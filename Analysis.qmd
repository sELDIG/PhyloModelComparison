...
env.metrics2 = rownames(agreement2)[agreement2[,1] >= 5]
dis.metrics2 = rownames(agreement2)[agreement2[,2] >= 7]
nic.metrics2 = rownames(agreement2)[agreement2[,3] >= 4]
mut.metrics2 = rownames(agreement2)[agreement2[,4] >= 7]
com.metrics2 = rownames(agreement2)[agreement2[,5] >= 6]
...
modelMetricDivergent(R2MatFull, pMatFull, x.labels = alt.x.labels, y.labels = c("Env. filtering", "Dispersal", "Niche conserv.", "Speciation", "Competition"),
             ...
             boxes2 = T, 
             boxProcesses2 = c(rep('env', length(env.metrics2)),
                                         rep('dis', length(dis.metrics2)),
                                         rep('mut', length(mut.metrics2))), 
             boxMetrics2 = c(env.metrics2, dis.metrics2, mut.metrics2), 
             lwd = 5, srt = 90, cex.asterisk = 2, metric.order = dend.order,
             lty2 = '42') # dash length 4, gap length 2
...
modelMetricContinous(R2MatFullBootSD,
                   cex.metrics = 1.4, cex.models = 1.3, cex.processes = 1.4,
              lwd = 5, srt = 90, metric.order = dend.order,
              x.labels = alt.x.labels,
              y.labels = c("Env. filtering", "Dispersal", "Niche conserv.", "Speciation", "Competition"))
mtext("Models", 1, outer = TRUE, cex = 2.5, at = .53)
#mtext("Metrics", 2, cex = 2.5, outer = TRUE, line = -3)
mtext("Processes", 3, cex = 2.5, outer = TRUE, at = 0.53)
dev.off()
...
dimnames(RFimportance)[[3]]  = c("ca", "fh", "ge", "hs", "am", "pw", "ve", "xe")
```
...
pdf('figures/Fig.4.pdf', height = 8.5, width = 14)
