...
empiricalModelInferencePlot = function(process, # env1, dis1, mut1, nic1, com1
  ...
  for (p in pairOrder) {
    ...
    mtext(tmp$cladeName[1:2], 1, at = 1:2, line = 2.5, cex = cex.clade, padj = 0.5)
    offset = metric.offset
    mtext(c(met1label, met2label, met1label, met2label), 1, at = c(1.1 - offset, 1.05 + offset, 2.05 - offset, 2. + offset), line = .5, cex = cex.metrics)
    ...
    # panel label
    text(.55, 0.97, paste0("(", panelLetters[panelIndex], ")"), cex = cex.panel)
    ...
    if (panelIndex %in% c(1, 4)) {
      mtext("Tree Metrics\n(quantiles)", 2, cex = cex.quantiles, line = 1.8, at = 0.37)
      mtext(paste("Inferred", processLabels[processes == process]), 2, cex = cex.process, line = 3, at = 2)
    }
...
bp1 = boxplot(log10(simuData$number_of_lineages) ~ simuData$model3, main = 'log # of lineages', col = modelColors$color[order(modelColors$newLabel)], xaxt = 'n', tck = -0.01, xlab = '', ylab = '')
axis(1, at = 1:8, labels = bp1$names, las = 2, cex.axis = 1.4, tck = -0.01)

for (i in statisticsIndices2) { 
  bp = boxplot(simuData[,i] ~ simuData$model3, main = names(simuData)[i], col = modelColors$color[order(modelColors$newLabel)], xaxt = 'n', tck = -0.01, xlab = '')
