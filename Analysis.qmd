...
simuData = allSimuData %>%
  ...
  dplyr::select(-tree_height, -crown_age)
...
# all 50 tree metrics (now removing mntd and mpd, mathematically redundant to 
#                      mean_branch_length_ext and psv, respectively)
statisticsIndices1 = which(names(simuData) %in% row.names(metricCors) &
                             !names(simuData) %in% c('mntd', 'mpd'))
...
png("figures/FigureS1_metric_correlations.png", height = 1200, width = 900)
par(mar = c(8, 8, 10, 8))
...
pdf('figures/all_metric_correlations.pdf', height = 15, width = 13)
...
# Plotting only the subset of metrics that are not redundant with each other
# or richness, aside from those commonly known 

# *NOTE: This figure using image.real3d is not currently working for this subset.*

# Whether or not to include tree metrics for this plot
# These were identified as redundant/correlated from the overall correlation plot
...
image.real3d(highlightMat, pMatHighlight, x.labels = alt.x.labels, 
             ...
             cex.metrics = 1.4, cex.models = 1.4, cex.processes = 1.6,
...
mtext("Models", 1, outer = TRUE, cex = 2.5, at = .53, line = 1.5)
...
envCP = crossPredictability(process = 'env', iterations = 100, allMetrics = TRUE)

comCP = crossPredictability(process = 'com', iterations = 100, allMetrics = TRUE)

nicCP = crossPredictability(process = 'nic', iterations = 100, allMetrics = TRUE)
...
empiricalModelInferencePlot('mut1', metric1='mean_branch_length_ext', metric2='laplac_spectrum_p',
                            ...
                            model.abbrev = F, legend = TRUE, cex.legend = 1.7, 
                            cex.legendpts = 2.5, cex.metrics = 1.1, 
                            metric.offset = .24, cex.clade = 1.3, 
                            cex.quantiles = 1.2, cex.panel = 2.5, 
                            cex.axis = 1.3, bar.space = .35, 
                            cex.process = 1.4, lwd = 4)
