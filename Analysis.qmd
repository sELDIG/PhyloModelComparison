...
crossPredictability = function(processIndex, modelsToExclude,
                               xCol = c("darkblue","blue", "lightblue", 
                                        "gray94", "pink","red", "darkred"),
                               cexValues = 1, cexMain = 2, cexAxis = 1.5,
                               cexLab = 1.8, mainText) {
  ...
  for(k in 1:length(modelsTMP)){
    ...
    for(j in 1:length(modelsTMP)){
      ...
      if(nrow(test) > 0 & k != j){
        y = predict(x, test)
        CpredictabilityR2[k,j] = cor(y$predictions, test[,1], method = "kendall")
      }
    }
  }
...
par(mfrow = c(1, 2), mar = c(5,8,4,4), mgp = c(3, 1, 0), oma = c(0, 0, 0, 1))

crossPredictability(processIndex = 4, modelsToExclude = 6, cexLab = 1.5,
                    cexAxis = 1.3, cexValues = 1, cexMain = 1.5,
                    mainText = "Mutation/speciation")
mtext("a)", 2, at= 1.2, las = 1, cex = 2.5, line = 2)

crossPredictability(processIndex = 2, modelsToExclude = 5, cexLab = 1.5,
                    cexAxis = 1.3, cexValues = 1, cexMain = 1.5,
                    mainText = "Dispersal")
