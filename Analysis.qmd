...
# Get clustering order of metric names for Figure 2
dissim = 1- corrs
distances = as.dist(dissim)
hc = hclust(distances)
dend = as.dendrogram(hc)
dend2 = seriate_dendrogram(dend, distances)
dend.order = order.dendrogram(dend2)
...
image.real3d <- function(mat, mat2, xCol = c("darkblue","blue", "lightblue", "gray94", "pink","red", "darkred"), 
  ...
  metricPosition = data.frame(metrics = rownames(mat)[dend.order], 
                              yPos = seq(0.5, nrow(mat)-0.5, by = 1))
  
  newMat = matrix(nrow = dim(mat)[1], ncol = 50)
  for(i in metric.order){   #1:dim(mat)[1]
  ...
  axis(2, at = metric.order, labels = x.labels, las = 2, cex.axis = cex.metrics)
  ...
  for(i in metric.order){    # 1:dim(mat2)[1]
...
image.real3d(R2MatFull, pMatFull, x.labels = alt.x.labels, y.labels = c("Env. filtering", "Dispersal", "Niche conserv.", "Speciation", "Competition"),
             ...
             boxMetrics = c(env.metrics, dis.metrics, mut.metrics), lwd = 5,
