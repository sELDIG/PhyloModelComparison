...
for (p in pairs) {
  ...
  tmpBarData = empiricalMetrics[empiricalMetrics$pair == p, c(met1, met2)] %>%
    rowwise() %>%
    mutate(quantile.met1 = sum(get(met1) > simuData[, met1], na.rm = T)/nrow(simuData),
           quantile.met2 = sum(get(met2) > simuData[, met2], na.rm = T)/nrow(simuData)) %>%
