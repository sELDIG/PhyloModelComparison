...
simuData = left_join(processDF, treeOutput, by = c('model', 'simID'))
...
```{r}
image.real3d <- function(mat, mat2, xCol = c("darkblue","blue", "cyan", "gray94", "coral","red", "darkred"), 
                       range = c(-1,1), x.labels = dimnames(mat)[[1]],      y.labels = dimnames(mat)[[2]], alpha = .01) { 
  ...
  axis(2, at = 1:nrow(mat), labels = x.labels, las = 2, cex.axis = .5)
  axis(3, at = seq(5, 45, by = 10), labels = y.labels, cex.axis = 1, las = 1)
  axis(1, at = xpos, labels = rep(dimnames(mat)[[3]], 5), cex.axis = 0.7, las = 2)
...
mtext("Metrics", 2, at = nStats/2, cex = 2, line = 8)
...
x = barplot(t(predictabilityR2CV), beside = T, las = 1, horiz = F, ylim = c(0,1.1), ylab = "Predicted-observed correlation", xaxt = "n", cex.lab = 1.8, cex.axis = 1.6, col = modelColors$color)
