...
## Plot metric-process correlations (Fig.1)
...
```{r}
RFimportancePlot <- function(mat, xCol = c( "gray94", "lightblue", "blue", "darkblue"), 
...
```{r, eval = chunkEval}
pdf('figures/Fig.S2-VariableImportance.pdf', height = 15, width = 13)
par(mfrow = c(1,1), mar = c(4,15,3,3), oma = c(3, 0, 2, 0))

RFimportancePlot(RFimportance,
                   cex.metrics = 1.4, cex.models = 1.3, cex.processes = 1.4,
              lwd = 5, srt = 90)
mtext("Models", 1, outer = TRUE, cex = 2.5, at = .53)
#mtext("Metrics", 2, cex = 2.5, outer = TRUE, line = -3)
...
## Plot predictability within a model
...
## Plot predictability between models

As a next step, we calculate show between models, i.e. we use the random forests tuned for one model to predict to the next model.
