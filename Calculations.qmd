...
```{r}
# strict measure of agreement where all models show the same sign and p < alpha for each
...
# less strict where all models show the same sign regardless of p
agreement2 = numAgreements[(numAgreements[,1] >= 5 ) |
                            (numAgreements[,2] >= 7 ) | 
                            (numAgreements[,3] >= 4 ) | 
                            (numAgreements[,4] >=7 ) | 
                            (numAgreements[,5] >= 6),]
...
resultsToSave$consistency$agreement2 = agreement2
...
We calculate within-model predictabilty on all metrics. In the script below
...
```{r}
speciationCP = crossPredictability(process = 'mut', num.trees = 5000, allMetrics = F)
dispersalCP = crossPredictability(process = 'dis', num.trees = 5000, allMetrics = F)

envCP = crossPredictability(process = 'env', num.trees = 5000, allMetrics = F)
comCP = crossPredictability(process = 'com', num.trees = 5000, allMetrics = F)
nicCP = crossPredictability(process = 'nic', num.trees = 5000, allMetrics = F)
```
